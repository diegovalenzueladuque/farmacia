...
if (isset($_GET['id'])) {
	...
	if (isset($_POST['enviar']) && $_POST['enviar'] == 'si') {
		...
		if (!$nombre) {
			...
			$mensaje = 'Ingrese correo del usuario';
		}/*elseif(!$clave){
			...
			$sql = $usuarios->editUsuario($id, $nombre, $email, $rol);
			//$sql = $usuarios->editPassword($id, $clave);
			//print_r($res);exit;
			if ($sql) {
				$msg = 'ok';
				header('Location: verusuario.php?m=' . $msg . '&id=' . $id);
			}
		}
	}
}
			...
			<div class="col-md-6 mt-3">
				...
				<form action="" method="post">
					<div class="form-group">
						...
						<input type="text" name="nombre" value="<?php echo $res['usuario']; ?>" placeholder="Nombre del usuario" class="form-control">
					</div>
					<div class="form-group">
						<label>Cambie su email</label>
						<input type="text" name="email" value="<?php echo $res['email']; ?>" placeholder="indique nuevo mail" class="form-control">
					</div>
